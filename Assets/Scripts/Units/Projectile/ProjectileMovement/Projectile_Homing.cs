...
public class Projectile_Homing : MonoBehaviourPun
{
    int oldtargetID =-1;
    ...
    public void SetHomingTarget(int pvID) {
        //    numBounce++;
      PhotonView targetPv =   PhotonNetwork.GetPhotonView(pvID);
        if (targetPv != null)
        {
    ...
    private void OnProjectileHit(EventObject eo)
    {
      ...
      //      Debug.Log("Hit and Change");
            oldtargetID = homingTarget.GetComponent<PhotonView>().ViewID;
            homingTarget = null;
