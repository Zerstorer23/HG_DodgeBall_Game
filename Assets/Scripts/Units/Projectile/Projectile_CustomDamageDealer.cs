...
public class Projectile_CustomDamageDealer : MonoBehaviourPun
{
    ...
    {
        int fieldNo = (int)photonView.InstantiationData[0];
        playerDict = GameFieldManager.GetPlayersInArea(fieldNo);
        playerList = playerDict.Keys.ToArray();// PlayerManager.GetPlayers();
    }
    private void Update()
    {
        FindNearByPlayers();
